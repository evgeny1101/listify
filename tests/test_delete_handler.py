...
from handlers.delete import parse_ids, cmd_del, on_ids_input, show_delete_confirm, DeleteNote
...
class TestCmdDel:
    ...
    @pytest.mark.asyncio
    async def test_cmd_del_without_args_shows_list(self, mock_message, mock_state, sample_notes):
        with patch("handlers.delete.get_notes", new_callable=AsyncMock) as mock_get:
    ...
    @pytest.mark.asyncio
    async def test_cmd_del_with_args_calls_confirm(self, mock_message, mock_state, sample_notes):
        mock_message.text = "/del 1, 2"
        
        with patch("handlers.delete.get_notes", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = sample_notes
            with patch("handlers.delete.show_delete_confirm", new_callable=AsyncMock) as mock_confirm:
                await cmd_del(mock_message, mock_state)
                mock_confirm.assert_called_once()

    @pytest.mark.asyncio
    async def test_cmd_del_invalid_ids_shows_error(self, mock_message, mock_state, sample_notes):
...
class TestOnIdsInput:
    @pytest.mark.asyncio
    async def test_on_ids_input_valid_ids_calls_confirm(self, mock_message, mock_state, sample_notes):
...
class TestShowDeleteConfirm:
    @pytest.mark.asyncio
    async def test_show_delete_confirm_valid_ids(self, mock_message, mock_state, sample_notes):
        with patch("handlers.delete.get_notes", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = sample_notes
    ...
    @pytest.mark.asyncio
    async def test_show_delete_confirm_invalid_ids_shows_error(self, mock_message, mock_state, sample_notes):
        with patch("handlers.delete.get_notes", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = sample_notes
            
            await show_delete_confirm(mock_message, [999], mock_state)
            
            mock_message.answer.assert_called_with("Неверные ID. Введите существующие номера записей.")

    @pytest.mark.asyncio
    async def test_show_delete_confirm_empty_ids_shows_error(self, mock_message, mock_state, sample_notes):
        with patch("handlers.delete.get_notes", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = sample_notes
